...
struct APIC_LVT {
    unsigned int vector :8, // 0~7 指定中斷向量號
                 deliver_mode :3, // 8~10 投遞方式
                 res_1 :1, // 11 保留
                 deliver_status :1, // 投遞狀態
                 polarity:1, //13 中斷輸入引腳的觸發極性
                 irr :1, //14 僅在fixed投遞模式下有效，LAPIC處理中斷時置位，收到EOI時復位。
                 trigger :1, //15 LINT0、LINT1引腳的觸發方式
                 mask :1, //16 屏蔽中斷
                 timer_mode :2, //17~18 設置定時器/計數器模式
                 res_2 :13; // 19~31 保留
...
struct IOAPIC_map {
    unsigned int physical_address; // RCBA暫存器保存的基地址。
    unsigned char *virtual_index_address; // 索引暫存器的線性地址。
    unsigned int *virtual_data_address; // 數據暫存器的線性地址。
    unsigned int *virtual_EOI_address; // EOI暫存器的線性地址。
} ioapic_map;
